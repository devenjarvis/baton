...
- **Focus Mode**: Two-section pipeline view with a SESSIONS section (all in-progress sessions, each with an inline status badge: error/waiting/finished/normal) and a REVIEW QUEUE section (agents that have finished and await review). Navigation between sections is tracked by the `focusCursorSection` enum (`focusSectionActive`, `focusSectionReview`). `focusLaunch` is a separate panel type for the fullscreen per-agent terminal; pressing space/enter in focus mode opens this view. Key state fields on the dashboard model: `focusModeActive bool`, `focusActiveIdx int`, `focusQueueIndex int`, `focusLaunchAgent *agent.Agent`. Wellness features: audio chimes are suppressed while focus mode is active; a session timer tracks elapsed time against `FocusSessionMinutes` and surfaces a break hint when the limit is reached; every session block is logged to `.baton/logs/wellness.log`. A soft agent limit (`MaxConcurrentAgents`, default 3) warns when exceeded — double-pressing `n` overrides the warning and spawns anyway. Configuration fields: `FocusModeEnabled bool`, `FocusSessionMinutes int`, `MaxConcurrentAgents int`, `MaxReviewBacklog int`.
