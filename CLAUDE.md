...
The dashboard is a single pipeline view (SESSIONS in progress + REVIEW QUEUE awaiting merge) with one cursor. There is no alternate layout, mode toggle, or "advanced" view — the pipeline is the product. When proposing changes, assume the user is running ≤3 agents and reviewing in batch, not streaming output across many panes.
